...
- (IBAction)loginAction:(id)sender {
    ...
    [BmobUser loginInbackgroundWithAccount:self.userNameTF.text andPassword:self.pwTF.text block:^(BmobUser *user, NSError *error) {
        ...
        if (user) {
          ...
          //登录环信
            [[EaseMobManager shareManager]loginWithWithName:user.username andPW:user.username andCallback:^(id obj) {
                
                if ([obj isKindOfClass:[NSDictionary class]]) {
                    NSLog(@"登录成功");
                }else{
                   [[EaseMobManager shareManager]registerWithName:user.username andPW:user.username andCallback:^(id obj) {
                       if (obj) {
                           NSLog(@"注册失败：%@",obj);
                       }else{
                           NSLog(@"注册成功！");
